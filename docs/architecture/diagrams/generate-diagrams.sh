...
check_plantuml() {
    if command -v plantuml >/dev/null 2>&1; then
        ...
        return 0
    elif java -jar /usr/local/bin/plantuml.jar --version >/dev/null 2>&1 2>/dev/null; then
...
generate_diagram() {
    ...
    if check_plantuml; then
        ...
        if generate_local "$input_file"; then
            ...
            return $(generate_online "$input_file")
        fi
    else
        # Use online PlantUML
        return $(generate_online "$input_file")
...
validate_diagrams() {
    ...
    while IFS= read -r -d '' file; do
        ((expected_count++))
    done < <(find "$PLANTUML_DIR" -name "*.puml" -print0)

    # Count generated diagrams
    if [ -d "$GENERATED_DIR" ]; then
        generated_count=$(find "$GENERATED_DIR" -name "*.png" -o -name "*.svg" | wc -l)
    fi
...
EOF
    ...
    if [ -d "$GENERATED_DIR" ]; then
        find "$GENERATED_DIR" -name "*.png" -o -name "*.svg" | sort | while read -r diagram; do
            local base_name
...
main() {
    ...
    while IFS= read -r -d '' puml_file; do
        ((total_count++))
        log_info "Processing $total_count/$(find "$PLANTUML_DIR" -name "*.puml" | wc -l): $(basename "$puml_file")"
