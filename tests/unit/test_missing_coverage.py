...
class TestMissingCoverage:
    ...
    def test_user_invalid_hash_validation(self) -> None:
        ...
        user = User(password_hash="invalid_hash_test")
        assert user.password_hash == "invalid_hash_test"
    ...
    def test_user_set_password_exception(self) -> None:
        ...
        with patch("bcrypt.hashpw", side_effect=Exception("Bcrypt error")):
            ...
            assert result.is_failure
            assert "Password hashing failed" in result.error

    def test_user_validate_strength_edge_cases(self) -> None:
        """Test models.py lines 173, 180-181 - password strength validation edge cases."""
        user = User()

        # Test various password strength scenarios
        assert user._validate_password_strength("") is False  # Empty password
        assert user._validate_password_strength("a") is False  # Too short
        assert user._validate_password_strength("aaaaaaaa") is False  # No uppercase
        assert user._validate_password_strength("AAAAAAAA") is False  # No lowercase
        assert user._validate_password_strength("Aa1") is False  # No special char
        assert user._validate_password_strength("Aa1!") is True  # Valid
    ...
    def test_user_password_model_edge_cases(self) -> None:
        ...
        assert user._validate_password_strength("ValidPass123!") is True
        assert user._validate_password_strength("invalid") is False
