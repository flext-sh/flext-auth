...
class TestEndToEndRealWorkflows:
    ...
    def test_complete_user_registration_workflow(self) -> None:
        ...
        registration_result = auth.create_user(
            "testuser", "test@example.com", strong_password
        )
        ...
        email_duplicate_result = auth.create_user(
            "testuser2", "test@example.com", strong_password  
        )
    ...
    def test_authentication_and_session_workflow(self) -> None:
        ...
        user_result = auth.create_user(username, email, password)
        ...
        claims = token_verify_result.data
        assert claims.username == username

        # Step 5: Try authentication with wrong password (should fail)
        wrong_auth_result = auth.authenticate(username, "WrongPassword123!")
    ...
    def test_account_lockout_workflow(self) -> None:
        ...
        user_result = auth.create_user(username, email, correct_password)
        assert user_result.success
        ...
        for attempt in range(3):
            fail_result = auth.authenticate(username, wrong_password) 
            assert not fail_result.success
            assert "invalid" in fail_result.error.lower() or "credentials" in fail_result.error.lower()
        ...
        locked_auth_result = auth.authenticate(username, correct_password)
    ...
    def test_password_security_workflow(self) -> None:
        ...
        for strong_password in strong_passwords:
            ...
            hashed_password = hash_result.data.value
            ...
            assert verify_result.data is False or wrong_verify.data is False
    ...
    def test_jwt_token_lifecycle_workflow(self) -> None:
        ...
        refresh_claims = refresh_verify.data
    ...
    def test_repository_persistence_workflow(self) -> None:
        ...
        for user in users:
            ...
            assert username_result.data is not None
            retrieved_user = username_result.data
        ...
        for i, user in enumerate(users):
            ...
            assert session_get.data is not None
            assert session_get.data.user_id == str(user.id)
    ...
    def test_complete_application_workflow(self) -> None:
        ...
        registration = auth.create_user(username, email, password)
        ...
        access_token = str(authentication.data["access_token"])
