...
- Primary facades: `FlextAuthApiKeyProvider`, `FlextAuthProtocols`, `FlextAuthRegistry`, `FlextAuthLdapProvider`, `FlextAuthPasswordHasher`, `FlextAuthProviderService` (+27 more)
...
2. Confirm parent MRO chain via `pyproject.toml` `dependencies` filtered by `flext-*` (excluding `flext-auth` self).
...
- Public extensions exposed by this project: `FlextAuthApiKeyProvider`, `FlextAuthProtocols`, `FlextAuthRegistry`, `FlextAuthLdapProvider`, `FlextAuthPasswordHasher`, `FlextAuthProviderService` (+27 more).
