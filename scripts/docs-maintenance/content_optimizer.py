...
class ContentOptimizer:
    ...
    def _analyze_structure(self, content: str, doc_file: Path) -> List[OptimizationSuggestion]:
    ...
    def _analyze_content_quality(self, content: str, doc_file: Path) -> List[OptimizationSuggestion]:
        """Analyze content quality."""
        suggestions: List[OptimizationSuggestion] = []
        ...
        for para in long_paragraphs[:3]:  # Limit to first 3 examples
            suggestions.append(OptimizationSuggestion(
                file_path=doc_file,
                line_number=0,
    ...
    def _analyze_technical_content(self, content: str, doc_file: Path) -> List[OptimizationSuggestion]:
        ...
        todo_markers = re.findall(r'(?i)(todo|fixme|hack|xxx|note).*?:?\s*(.+)?', content)
    ...
    def _check_common_issues(self, content: str, doc_file: Path, lines: List[str]) -> List[OptimizationSuggestion]:
        ...
        for i, line in enumerate(lines):
            ...
            if re.match(r'^\s*[-*+]\s+', line):
                ...
                has_numbered = any(re.match(r'^\s*\d+\.', l) for l in context)
                if has_numbered:
                    suggestions.append(OptimizationSuggestion(
                        file_path=doc_file,
                        line_number=line_num,
            ...
            emphasis_issues = re.findall(r'(\*{1,3}[^*]*\*{0,2}(?!\*))|(\*{0,2}[^*]*\*{1,3})', line)
    ...
    def _check_enhancement_opportunities(self, content: str, doc_file: Path) -> List[OptimizationSuggestion]:
        ...
        if any(keyword in content.lower() for keyword in ['api', 'implementation', 'status', 'version']):
    ...
    def _has_table_of_contents(self, content: str) -> bool:
        ...
        return any(re.search(pattern, content_lower, re.IGNORECASE | re.MULTILINE) for pattern in toc_patterns)

    def _check_heading_hierarchy(self, headings: List[Tuple[str, str]]) -> List[Tuple[int, str]]:
    ...
    def discover_docs_files(self) -> List[Path]:
        ...
        active_docs = [doc for doc in docs_files if "archive" not in str(doc)]
    ...
    def run_optimization_audit(self) -> Dict[str, Any]:
        ...
        for doc_file in docs_files:
            ...
            self.metrics[str(doc_file.relative_to(self.project_root))] = self._calculate_metrics(suggestions)
        ...
        print(f"✅ Optimization audit complete! Found {len(all_suggestions)} suggestions across {len(docs_files)} files")
        return results

    def _calculate_metrics(self, suggestions: List[OptimizationSuggestion]) -> ContentMetrics:
        """Calculate content quality metrics for a file."""
        # Simple heuristic-based scoring
        structure_score = 100 - len([s for s in suggestions if s.content_type == "structure"]) * 10
        readability_score = 100 - len([s for s in suggestions if s.content_type == "readability"]) * 5
        completeness_score = 100 - len([s for s in suggestions if s.content_type == "content"]) * 15
        technical_score = 100 - len([s for s in suggestions if s.content_type == "technical"]) * 10

        overall_quality = (structure_score + readability_score + completeness_score + technical_score) / 4
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            "summary": {
                ...
                "average_quality_score": sum(m.overall_quality for m in audit_results["quality_metrics"].values()) / max(len(audit_results["quality_metrics"]), 1)
            },
    ...
    def _generate_recommendations(self, audit_results: Dict[str, Any]) -> List[str]:
        ...
        if auto_fixable > 0:
            recommendations.append(f"🤖 AUTOMATION: {auto_fixable} issues can be auto-fixed")

        severity_breakdown = audit_results["suggestions_by_severity"]
        if severity_breakdown.get("high", 0) > 0:
            recommendations.append(f"🔴 PRIORITY: Address {severity_breakdown['high']} high-severity issues first")

        if avg_quality < 70:
            recommendations.append(f"🟡 IMPROVE: Overall documentation quality needs attention ({avg_quality:.1f}/100)")
        elif avg_quality > 90:
            recommendations.append("✅ EXCELLENT: High content quality standards maintained")

        type_breakdown = audit_results["suggestions_by_type"]
        if type_breakdown.get("structure", 0) > 5:
            recommendations.append("🏗️ STRUCTURE: Multiple files need structural improvements")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="FLEXT Auth Content Optimization System")
