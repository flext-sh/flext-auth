...
class LinkValidator:
    ...
    def __init__(self, project_root: Path, timeout: int = 10, max_retries: int = 3):
    ...
    def validate_internal_link(self, from_file: Path, url: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            if url.startswith('#'):
                ...
                headings = re.findall(r'^#{1,6}\s+(.+)$', content, re.MULTILINE | re.IGNORECASE)
            ...
            return False, f"Error checking link: {str(e)}"

    def validate_image_link(self, from_file: Path, url: str) -> Tuple[bool, Optional[str]]:
            ...
            if url.startswith('http'):
                ...
                if image_path.exists() and image_path.is_file():
                    ...
                    if image_path.suffix.lower() in valid_extensions:
                        ...
                        return False, f"File exists but not a recognized image format: {image_path.suffix}"
                else:
    ...
    async def audit_document_links(self, doc_file: Path) -> List[LinkIssue]:
        ...
        try:
                ...
                if url.startswith('http'):
                    ...
                    if result.status != "valid":
                        ...
                        if result.status == "not_found":
                            suggestion = "Check if URL is correct or update to new location"
                        elif result.redirect_url:
                            suggestion = f"Update to redirect URL: {result.redirect_url}"
            ...
            issues.append(LinkIssue(
                ...
                severity="high",
                suggestion=f"Error processing file: {str(e)}"
            ))
    ...
    async def run_link_audit(self) -> Dict[str, Any]:
        ...
        async def audit_with_semaphore(doc_file: Path):
            async with semaphore:
                issues = await self.audit_document_links(doc_file)
                return issues
        ...
        print(f"✅ Link audit complete! Found {len(all_issues)} issues across {len(docs_files)} files")
        return self.results
    ...
    def _calculate_statistics(self):
        ...
        for issue in issues:
            if hasattr(issue, 'response_time') and issue.response_time:
                response_times.append(issue.response_time)

        validation_results["statistics"] = {
            "total_links_checked": sum(len(issues_list) for issues_list in validation_results["issues_by_type"].values()),
            "broken_links": len(validation_results["issues_by_type"].get("broken_external_link", [])) +
                           len(validation_results["issues_by_type"].get("broken_internal_link", [])),
            "broken_images": len(validation_results["issues_by_type"].get("broken_image_link", [])),
            "accessibility_issues": len(validation_results["issues_by_type"].get("missing_alt_text", [])),
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            "summary": {
                ...
                "total_files_audited": self.results["validation_results"]["total_files_audited"],
                "total_issues_found": self.results["validation_results"]["total_issues_found"],
                "quality_score": self._calculate_quality_score()
            },
            "issues_by_severity": self.results["validation_results"]["issues_by_severity"],
            "issues_by_type": {k: len(v) for k, v in self.results["validation_results"]["issues_by_type"].items()},
    ...
    def _calculate_quality_score(self) -> int:
        ...
        accessibility_penalty = (stats["accessibility_issues"] / total_links) * accessibility_weight
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if stats["broken_links"] > 0:
            recommendations.append(f"🔴 CRITICAL: Fix {stats['broken_links']} broken links immediately")

        if stats["broken_images"] > 0:
            recommendations.append(f"🟡 IMPORTANT: Fix {stats['broken_images']} broken image links")

        if stats["accessibility_issues"] > 0:
            recommendations.append(f"🔵 ACCESSIBILITY: Add alt text to {stats['accessibility_issues']} images")

        if stats["files_with_issues"] > 5:
            recommendations.append(f"📊 CONSIDER: {stats['files_with_issues']} files have link issues - review systematically")
