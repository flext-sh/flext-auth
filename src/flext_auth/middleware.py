"""Authentication middleware adapters for HTTP clients and web applications.
...
Integration Pattern:
    ...
    auth = HttpAuthMiddleware(JwtAuthProvider(secret="key"))
    ...
    auth = WebAuthMiddleware(OAuth2AuthProvider(...))
...
class WebAuthMiddleware:
    ...
    def process_request(
        ...
        try:
            ...
            if any(
                request_path.startswith(excluded) for excluded in self._exclude_paths
            ):
                self._logger.debug(
    ...
    def _extract_token(self, request: object) -> str | None:
        ...
        if isinstance(headers, dict):
            ...
            auth_header = (
                headers.get(self._header_name, "") if hasattr(headers, "get") else ""
            )

        if auth_header:
            # Strip prefix if present
            if self._token_prefix and auth_header.startswith(f"{self._token_prefix} "):
