...
class FlextAuthVersion(FlextProjectVersion):
    """Structured metadata for the flext auth distribution."""

    @classmethod
    def current(cls) -> FlextAuthVersion:
        """Return canonical metadata loaded from pyproject.toml."""
        return cls.from_metadata(_metadata_obj)


VERSION: Final[FlextAuthVersion] = FlextAuthVersion.current()
...
__all__ = ["VERSION", "FlextAuthVersion", "__version__", "__version_info__"]
